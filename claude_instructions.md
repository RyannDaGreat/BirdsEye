...
| **Cross-Component Field Highlighting** | `hoveredFields` store (Set of field keys) syncs highlighting across ALL field bars (stats panel, detail panel, anywhere), filter histograms, and SPLOM. The coupling is built into `FieldBar.svelte` itself via the `fieldKey` prop — any FieldBar with `fieldKey` set automatically sets/clears `hoveredFields` on hover and reacts to external changes with `.highlighted` class. ONE behavior, ONE component — not per-parent wiring. SPLOM also subscribes and highlights matching rows/columns. |
