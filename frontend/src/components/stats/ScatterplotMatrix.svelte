<!--
  ...
  One big canvas. Draws everything in a single pass. CSS scales to fit.
-->
<script>
  ...
  async function render() {
    ...
    for (let row = 0; row < n; row++) {
      for (let col = 0; col < n; col++) {
        ...
        ctx.clip();
        // Translate so drawing functions work in 0,0 → CELL,CELL space
        ctx.translate(ox, oy);
    ...
    // Column labels (45° diagonal, above grid)
    ctx.font = '9px ' + getComputedStyle(document.body).fontFamily;
    ctx.textAlign = 'left';
    for (let i = 0; i < n; i++) {
      const x = PAD_LEFT + i * CELL + CELL / 2;
      const y = PAD_TOP - 4;
      ctx.save();
      ctx.translate(x, y);
      ctx.rotate(-Math.PI / 4);
      ctx.fillStyle = hoverCol === i ? '#4a9eff' : '#888';
      ctx.fillText(fieldLabel(fields[i].key), 0, 0);
      ctx.restore();
    }

    // Row labels (horizontal, left of grid)
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
    for (let i = 0; i < n; i++) {
      const x = PAD_LEFT - 4;
      const y = PAD_TOP + i * CELL + CELL / 2;
      ctx.fillStyle = hoverRow === i ? '#4a9eff' : '#888';
      ctx.fillText(fieldLabel(fields[i].key), x, y);
    }
  }

  $: if (fields && n >= 0) render();
  $: if (useLog !== undefined) render();
  $: if (hoverRow >= -1) render();  // re-render on hover change

  $: scale = getScale();
  // Recompute scale on fields change
  ...
  function onCanvasLeave() { hoverRow = -1; hoverCol = -1; hoverInfo = ''; }
</script>
